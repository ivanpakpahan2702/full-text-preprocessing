...
function saveFileAs() {
    if (promptFilename = prompt("Simpan file sebagai", "")) {
        var textBlob = new Blob([document.getElementById("txtAreaResult").value], { type: 'text/plain' });
...
$(function () {
    $("#process").click(function () {
        ...
        if ((($('#slangFile').get(0).files.length === 0) && ($("#normalizationChecked").attr("slangCheck") == "1"))) {
        ...
        else if (($('#stopwordFile').get(0).files.length === 0) && ($("#stopwordChecked").attr("stopCheck") == "1")) {
            ...
            const Data_Array = [document.getElementById('slangFile').files[0], document.getElementById('stopwordFile').files[0]];

            Data_Array.forEach((value) => {
                full_data.append('files[]', value);
            });

            full_data.append("case_folding_stat", $("#caseFoldingChecked").attr("caseCheck"));
            full_data.append("normalization_stat", $("#normalizationChecked").attr("slangCheck"));
                ...
                error: function (x, t, m) {
                    if (t === "timeout") {
                        ...
                        Swal.fire({
                            ...
                            icon: "error",
                            footer: "Download Format <a href='/static/formats/format_file.rar'>&nbsp;Disini</a>",
                            confirmButtonText: "Tutup",
                        });
                    }
                },
